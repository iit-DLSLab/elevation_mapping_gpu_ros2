...
ElevationMappingNode::ElevationMappingNode(const rclcpp::NodeOptions& options)
  ...
  // mapFrameId_ = "odom_lidar";
  // baseFrameId_ = "robot";
  // correctedMapFrameId_ = "odom_lidar";
  ...
  std::cout << "map frame: " << mapFrameId_ << std::endl;
  ...
  initialize_tf_offset_ = std::vector<double>(4, 0.0); 
    ...
    for (const auto& sub_name : unique_sub_names) {  
      ...
      if(this->get_parameter("subscribers." + sub_name + ".data_type", data_type)){
          ...
          if(data_type == "image"){
            ...
            this->get_parameter("subscribers." + sub_name + ".camera_info_topic_name", info_topic);
...
void ElevationMappingNode::inputPointCloud(const sensor_msgs::msg::PointCloud2::ConstSharedPtr cloud,
    ...
    std::cout << "inputh point cloud: " << std::endl;
